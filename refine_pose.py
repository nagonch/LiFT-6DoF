...
def refine_splat_camera_poses(
    ...
    for epoch in tqdm(range(config.num_epochs), desc="Training Epochs"):
        ...
        if epoch % 50 == 0 and config.verbose:
            loss_img = mask_aware_loss_img(loss_fn, rendered, alphas, images, masks)
            loss_img = loss_img ** (0.1)
            plt.imshow(loss_img[-1].detach().cpu().numpy(), cmap="gray")
            plt.savefig(f"result_imgs/{epoch:04d}_loss_img.png")
            plt.close()
            plt.imshow(rendered[-1].detach().cpu().numpy())
            plt.savefig(f"result_imgs/{epoch:04d}_rendered.png")
            plt.close()
            plt.imshow(images[-1].detach().cpu().numpy())
            plt.savefig(f"result_imgs/{epoch:04d}_target.png")
            plt.close()
            plt.plot(losses_log)
            plt.savefig("refine_loss.png")
            plt.close()
            plt.plot(translation_log_x)
            plt.plot(
                [
                    pose_vector_gt[0].cpu().numpy(),
                ]
                * epoch,
                label="gt",
            )
            plt.savefig("translation_log_x.png")
            plt.close()

            plt.plot(translation_log_y)
            plt.plot(
                [
                    pose_vector_gt[1].cpu().numpy(),
                ]
                * epoch,
                label="gt",
            )
            plt.savefig("translation_log_y.png")
            plt.close()

            plt.plot(translation_log_z)
            plt.plot(
                [
                    pose_vector_gt[2].cpu().numpy(),
                ]
                * epoch,
                label="gt",
            )
            plt.savefig("translation_log_z.png")
            plt.close()

            plt.plot(rel_rotation_log_x)
            plt.plot(
                [
                    pose_vector_gt[3].cpu().numpy(),
                ]
                * epoch,
                label="gt",
            )
            plt.savefig("rel_rotation_log_x.png")
            plt.close()

            plt.plot(rel_rotation_log_y)
            plt.plot(
                [
                    pose_vector_gt[4].cpu().numpy(),
                ]
                * epoch,
                label="gt",
            )
            plt.savefig("rel_rotation_log_y.png")
            plt.close()

            plt.plot(rel_rotation_log_z)
            plt.plot(
                [
                    pose_vector_gt[5].cpu().numpy(),
                ]
                * epoch,
                label="gt",
            )
            plt.savefig("rel_rotation_log_z.png")
            plt.close()
